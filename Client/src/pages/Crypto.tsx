    ...
    <section  className='max_padd_container pt-24'>
      <table className='w-full mx-auto'>
        ...
        <tbody>
          {
            data.map((coin)=>(
              <tr key={coin.name} id={coin.id} className='border-b  border-r-slate-900/50 text-gray-400 p-6 medium-14 text-center'>
                <td className='text-left'>{coin.market_cap_rank}.  {coin.symbol} </td>
                <td className='flexCenter py-4'><div className='sm:w-[30%] flex items-center text-left mx-auto gap-x-3 text-gray-400'><img src={coin.image} alt="coninimg" height={30} width={30} />{coin.name}</div></td>
                {/* <td>{coin.symbol}</td> */}
